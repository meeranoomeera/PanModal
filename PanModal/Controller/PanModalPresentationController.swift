	...
	private lazy var backgroundView: DimmedView = {
		...
		view.didTap = { [weak self] _ in
			if self?.presentable?.allowsTapToDismiss == true {
				self?.presentable?.panModalWillDrop(dropWay: .tapOnBackground)
		...
		default:
			...
			if isVelocityWithinSensitivityRange(velocity.y) {
				...
				if velocity.y < 0 {
					...
					presentable?.panModalWillDrop(dropWay: .swipe)
...
private extension PanModalPresentationController {
	...
	func trackScrolling(_ scrollView: UIScrollView) {
		...
		scrollView.showsVerticalScrollIndicator = presentable?.isPanScrollIndicatorVisible ?? true
	}
...
private extension PanModalPresentationController {
	...
	private func wrapInWrapperContainerIfNeeded() {
		...
		containerWrapperView.isTouchPossible = { [weak self, weak containerView, weak presentable] point, event in
			...
			switch presentable.isUserInteractionEnabled {
			case .onlyWithPresentedView:
				let containerFrame = CGRect(
					x: containerView.frame.minX,
					y: panContainerView.frame.minY,
					width: panContainerView.frame.width,
					height: panContainerView.frame.height
				)
